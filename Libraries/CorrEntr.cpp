...
double Norm(double* x1, double* x2, int l)
{
    int i;
    float s = 0.;
    for (i = 0; i < l; i++)
...
double MiMa(double mat[nw][nw], int l, char q) //q = 'i' -> min; q = 'a' ->max
{
    int i,j;
    double extr;
    extr = mat[0][1];
...
double CorrEntr(double* ser, int n)  //int для размерности вложения
{
    int i, j, k, l, color=0;
    double ls, step, mi, ma, c, dc, d0=0., ent=1.;
    double x1[nw], x2[nw], cn[21], cl[21], ll[21];
